...
pub fn load_embedded_level() -> Level {
    let level_file = TEXTURES_DIR
        .get_file("level.amap")
        .unwrap_or_else(|| panic!("Failed to find embedded level.amap"));

    level_from_binary(level_file.contents())
        .unwrap_or_else(|e| panic!("Failed to decode embedded level.amap: {e}"))
}

fn level_from_binary(data: &[u8]) -> Result<Level, String> {
    ...
    if data.len() < required {
        return Err(format!(
            "File too small for {width}×{height} level: need {required}, got {}",
            data.len()
        ));
    }
    ...
    for y in 0..height {
        ...
        for x in 0..width {
            ...
            let _wall_height = data[base + 2]; // wire into Level when ready
            let _prop_type = data[base + 4]; // wire into Level when ready
            let _special_type = data[base + 6]; // wire into Level when ready
            let _special_data = data[base + 7]; // wire into Level when ready
